function generateSphere(x, y, z, radius, segments) {
    ...
    for (var i = 0; i <= segments; i++) {
        ...
        var secLat = (1 / Math.cos(latAngle))
        var tanLat = Math.tan(latAngle)

        for (var j = 0; j <= segments; j++) {
            var lonAngle;
            if (j <= segments / 2) {
                lonAngle = Math.PI * (-0.5 + (j / segments));
            } else {
                lonAngle = Math.PI * (1.5 + (j / segments));
            }
            var sinLon = Math.sin(lonAngle);
            var cosLon = Math.cos(lonAngle);

            var xCoord = cosLon * tanLat;
            var yCoord = sinLon * tanLat;
            var zCoord = secLat;
