function generateSphere(x, y, z, radius, segments) {
    ...
    for (var i = 0; i <= segments; i++) {
        var latAngle = Math.PI * (i / segments);
        var vLat = latAngle;

        for (var j = 0; j <= segments; j++) {
            var lonAngle = 2 * Math.PI * (j / segments);
            var tanLon = Math.tan(lonAngle);
            var secLon = (1/Math.cos(lonAngle));

            var xCoord = tanLon * vLat;
            var yCoord = secLon * vLat;
            var zCoord = Math.pow(vLat,2);
...
function main() {
    ...
    var tubeData = generateSphere(0, 0, 0.5, 0.5, 100);    // Example sphere: x=0, y=0, z=0.5, radius=0.5, segments=100
