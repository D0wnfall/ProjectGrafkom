function generateSphere(x, y, z, radius, segments, scaleX, scaleY, scaleZ) {
    var vertices = [];
    var colors = [];
    var textureCoords = [];
...
function main() {
    ...
    var tubeData = generateSphere(0, 0, 0.5, 0.8, 50, 1 , 1 ,1);
    ...
    image.src = 'ressources/Jupiter.png'; 
    ...
    var ring = generateSphere(2, 0, 0.5, 0.8, 50, 1 , 1 ,0);
    ...
    GL.bufferData(GL.ARRAY_BUFFER, new Float32Array(ring.colors), GL.STATIC_DRAW);

    var TUBE_TEXTURE_COORDS2 = GL.createBuffer();
    GL.bindBuffer(GL.ARRAY_BUFFER, TUBE_TEXTURE_COORDS2);
    GL.bufferData(GL.ARRAY_BUFFER, new Float32Array(ring.textureCoords), GL.STATIC_DRAW);

    var TUBE_FACES2 = GL.createBuffer();
    GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, TUBE_FACES2);
    GL.bufferData(GL.ELEMENT_ARRAY_BUFFER, new Uint16Array(ring.faces), GL.STATIC_DRAW);
    ...
    var animate = function (time) {
        ...
        GL.drawElements(GL.TRIANGLES, ring.faces.length, GL.UNSIGNED_SHORT, 0);
        GL.flush();
